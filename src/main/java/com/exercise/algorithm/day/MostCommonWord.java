 ...
 * @Description 最常见的单词
 * @Author yeqiang
 * @Date 2022/4/17 11:06 PM
 * @Version 1.0
 **/
public class MostCommonWord {

    public String mostCommonWord(String paragraph, String[] banned) {
        String splitPara = paragraph.replaceAll("[!]", " ").
                replaceAll("[?]", " ").
                replaceAll("[.]", " ").
                replaceAll("[']", " ").
                replaceAll("[,]"," ").
        ...
        for (String word:splitPara.split(" ")){
            countWordMap.put(word.toLowerCase().trim(),countWordMap.getOrDefault(word.toLowerCase().trim(), 0)+1);
        }
        int count = 0;
        String res = "";
        for (Map.Entry<String, Integer> map:countWordMap.entrySet()){
            String word = map.getKey();
            if (word.equals("")){
                continue;
            }
            Integer curCount = map.getValue();
            boolean bannerStatus = false;
            for (String bannerWord:banned){
                if (word.equals(bannerWord)){
                    bannerStatus = true;
                    break;
                }
            }
            if (!bannerStatus && curCount > count){
                count = curCount;
                res = word;
            }
        }
