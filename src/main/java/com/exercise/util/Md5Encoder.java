...
public abstract class Md5Encoder {
    ...
    public static String encodeBase64(byte[] data, boolean urlSafe){
        byte [] md5Code = rawEncode(data);
        if(md5Code != null){
            return new String(urlSafe? Base64.encodeBase64URLSafe(md5Code): Base64.encodeBase64(md5Code),0,22);
        } else {
            return null;
        }
    }
    ...
    public static String encodePasswordWithDoubleMd5(String data,String salt){
        return encodePasswordAsSpringSecurity(encode(data) , salt);
    }

    public static void main(String[] args) {
        String origin = new String(Base64.decodeBase64("d2Vsb29rbmljZQ=="));
